...
async def _give_link(bot, uid: int):
    if not LOG_CHANNEL_ID:
        return "⚠️ Public logs aren't configured yet.", None
    ...
    if last and (now - last).total_seconds() < 86400:
        ...
        return (f"🕒 You already generated a link today.\nTry again in "
                f"<b>{left // 3600}h {(left % 3600) // 60}m</b>.", None)
    try:
        link = await bot.create_chat_invite_link(
            LOG_CHANNEL_ID, expire_date=now + timedelta(hours=24), member_limit=1)
    except Exception as exc:  # noqa: BLE001
        logger.warning("invite link failed: %s", exc)
        return "❌ Couldn't generate a link right now.", None
    await db.safe_update("users", {"user_id": uid}, {"$set": {"last_invite_at": now}})
    return (f"🔗 <b>Your private access link</b>\n\n{link.invite_link}\n\n"
            "<i>Valid 24h · single use.</i>", None)
